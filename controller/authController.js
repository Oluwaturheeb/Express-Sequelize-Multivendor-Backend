...
import { Roles, Users, seq } from '../conf/db.js';
...
export const register = async (req, res) => {
  ...
  if (!email && !password && !name) res.sendStatus(412).json({ message: 'Action required!' });
  else {
    try {
...
export const confirmEmail = (req, res, next) => {
  try {
    ...
    let user = Users.update({ verify: 'email' }, {
      where: {
        email: token
      }
    });
...
export const google = async (req, res) => {
  ...
  try {
    ...
    if (!newUser) res.json({code: 0, message: 'Can not create account!'});
    else {
      if (created) {
        await Roles.create({
          id: uuidv7(),
        ...
        res.json({code: 1, message: 'Welcome '+ newUser.name, user: newUser});
      } else res.json({code: 1, message: 'Welcome '+ newUser.name, user: newUser});
    }
  } catch (e) {
    res.status(500).json({code: 0, message: 'Unknown error!' + e.message});
  }
};
